...
public class ReviewService {
	...
	public List<Review> selectList() throws Exception{
		...
		List<Review> rList = dao.selectReview(conn);
		conn.close();
		return rList;
	 ...
	 * @throws Exception
	 */
	public int insertReview(Review review, List<Attachment> rList, int boardType) throws Exception{
		Connection conn = getConnection();
		int result = 0;
		...
		if(boardNo >0) {
			...
			//---------------------------------------------------글 삽입까지 성공
			  ...
			  if(result>0 && !rList.isEmpty() ) {
		        	result = 0; // result 재활용
		        	
		        	// fList에 저장된 파일정보를 하나씩 DB에 삽입
		        	for(Attachment at : rList) {
		        		// 게시글 번호를 파일 정보에 세팅
